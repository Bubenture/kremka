...
function sendScoreToGoogleSheets() {
    ...
    var scriptUrl = 'https://script.google.com/macros/s/AKfycbzkkKL6sSuc23oBw6JR50DH5m9ZgcDTkJ4FDMCUVGS47nRr5q5FOSR_qt5M2omS8_Hk3w/exec'; 
    
    // Данные для отправки (в данном случае счет)
    var data = {
        score: score  // Предполагается, что `score` - это глобальная переменная, содержащая текущий счет
    };
    ...
    .catch(function(error) {
        console.error('Ошибка отправки счета в Google Таблицу:', error);

    });
}

// Обработчик события для кнопки "records"
